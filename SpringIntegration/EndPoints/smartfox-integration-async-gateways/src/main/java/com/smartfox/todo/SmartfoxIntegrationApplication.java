...
public class SmartfoxIntegrationApplication implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) throws Exception {
		...
		Person baka = new Person("baka", "baka");
		List<Person> persons = Arrays.asList(person,baka);
		Message<?> message = MessageBuilder.withPayload(person).setHeader(MessageHeaders.REPLY_CHANNEL, "outputChannel").build();
		enhancedPrinterGateway.print(person);// NO LONGER SEND A MESSAGE - instead send a person
		
		// Not in ordered because of async
		for (int i = 0; i < persons.size(); i++) {
			ListenableFuture<String> capitalisedFuture = enhancedPrinterGateway.uppercase(persons.get(i));
			capitalisedFuture.addCallback(new ListenableFutureCallback<String>() {
